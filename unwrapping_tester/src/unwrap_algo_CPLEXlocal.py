...
class CPLEXlocalAlgo(UnwrappingAlgo):
    ...
    def unwrap(self, w: np.array, unwrap_param: dict):
        ...
        objective = unw.sum(
            (g[k] - g[h]) ** 2 
            for k in timeline_index 
            for h in range(max(k - d, min(timeline_index)), min(k + d + 1, max(timeline_index) + 1)) 
            if h in timeline_index and h != k
        )
